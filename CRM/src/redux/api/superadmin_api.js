...
export class SuperadminService {
  ...
  async getUsers(params) {
    ...
    try {
      const response = await axios.get('http://server-tttn.railway.internal:3001/api/superadmin/users', {
        headers: {
          'x-auth-token': token
        },
        params: {
  ...
  async blockUnblockUser(userId, isBlocked) {
    ...
    const token = state.auth.token; // Giả sử token được lưu trong state.auth.token
    try {
      const response = await axios.patch(`http://server-tttn.railway.internal:3001/api/superadmin/block-unblock-user/${userId}`, {
        isBlocked
      }, {
        headers: {
          'x-auth-token': token
