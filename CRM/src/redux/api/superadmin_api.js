...
export class SuperadminService {
  ...
  async getUsers(params) {
    ...
    try {
      const response = await axios.get('https://servertttn-production.up.railway.app/api/superadmin/users', {
        headers: {
          'x-auth-token': token
        },
        params: {
  ...
  async blockUnblockUser(userId, isBlocked) {
    ...
    const token = state.auth.token; // Giả sử token được lưu trong state.auth.token
    try {
      const response = await axios.patch(`https://servertttn-production.up.railway.app/api/superadmin/block-unblock-user/${userId}`, {
        isBlocked
      }, {
        headers: {
          'x-auth-token': token
