...
exports.getBanners = async (req, res) => {
  ...
  res.json({ data: { banners, totalCount } });
};
...
exports.flipAdminBusinessApproval = async (req, res) => {
  try {
    ...
    if (!businessInfo) {
      return res
        .status(404)
        .json({ error: "No business information available" });
    }
    ...
    businessInfo.citizenshipFront = convertToObjectId(
      businessInfo.citizenshipFront
    );
    businessInfo.citizenshipBack = convertToObjectId(
      businessInfo.citizenshipBack
    );
    businessInfo.businessLicence = convertToObjectId(
      businessInfo.businessLicence
    );
...
exports.getCategories = async (req, res) => {
  ...
  res.json({ data: { categories, totalCount } });
};
...
exports.deleteCategory = async (req, res) => {
  try {
    ...
    if (hasProducts) {
      return res.status(400).json({
        error:
          "Cannot delete category that has associated products. Please remove or reassign products first.",
      });
    }
...
exports.deleteBrand = async (req, res) => {
  try {
    ...
    if (hasProducts) {
      return res.status(400).json({
        error:
          "Cannot delete brand that has associated products. Please remove or reassign products first.",
      });
    }
  ...
  try {
    ...
    const analytics = {
      daily: {
        ...
        salesVsCustomers:
          newCustomersToday > 0
            ? (totalSalesToday[0]?.total || 0) / newCustomersToday
            : 0,
      ...
      monthly: {
        ...
        salesVsCustomers:
          newCustomersMonth > 0
            ? (totalSalesMonth[0]?.total || 0) / newCustomersMonth
            : 0,
